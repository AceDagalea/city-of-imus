...
export default function PastMayorsContent() {
  ...
  return (
    <article className="overflow-hidden rounded-xl border border-gray-100 bg-white shadow-card">
      ...
      <div className="relative aspect-[16/6] w-full bg-tenant-gray">
        <Image
      ...
      <div className="overflow-x-auto p-4 md:p-6">
        <table className="w-full min-w-[520px] border-collapse text-sm">
          <thead>
            <tr className="bg-tenant-navy text-left text-white">
          ...
          <tbody>
            {PAST_MAYORS.map((row, index) => (
              <tr
                key={`${row.year}-${index}`}
                className={index % 2 === 0 ? "bg-white" : "bg-tenant-gray/60"}
              >
                <td className="border-t border-gray-100 px-4 py-2.5 text-tenant-navy">
                  {row.name ?? "—"}
                </td>

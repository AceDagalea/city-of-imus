...
export default function GadDocumentCard({ document }: GadDocumentCardProps) {
  ...
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-xl border border-gray-100 bg-white shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:border-imus-navy/15 hover:shadow-lg">
      <Link href={getGadHtmlUrl(document.id)} className="flex h-full flex-col focus-ring rounded-xl">
        <div className="relative aspect-[4/3] overflow-hidden bg-imus-gray">
          {thumbnail ? (
            <Image
              src={thumbnail}
              alt={document.title}
        ...
        <div className="flex flex-1 flex-col p-4">
          <h3 className="line-clamp-3 font-heading text-base font-bold leading-snug text-imus-navy">
            {document.title}
          </h3>

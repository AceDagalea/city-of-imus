...
import type { BanaagIssue } from "@/lib/banaag-generated";

interface BanaagIssueViewProps {
  issue: BanaagIssue;
}

export default function BanaagIssueView({ issue }: BanaagIssueViewProps) {
        ...
        {issue.pages.map((src, index) => (
          ...
          >
            <figure className="mx-auto max-w-4xl">
              <Image
                src={src}
                alt={`${issue.title} — ${language === "fil" ? "Pahina" : "Page"} ${index + 1}`}
                width={1200}
                height={1600}
